...
import { useEditJobByIdMutation, useGetJobByIdQuery } from "@/redux/api/jobApi";
...
const inputClass =
  "w-full border px-3 py-2 rounded-md focus:outline-none focus:ring focus:border-blue-300";
...
const EditJob = () => {
  ...
  const [editJob, { isLoading: isSubmitting }] = useEditJobByIdMutation();

  const [isClient, setIsClient] = useState(false);
  const [questions, setQuestions] = useState([]);
  const [isQuestionModalOpen, setIsQuestionModalOpen] = useState(false);

  // Local state to reflect UI values
  ...
  const openAddQuestionsModal = () => setIsQuestionModalOpen(true);
  const closeAddQuestionsModal = () => setIsQuestionModalOpen(false);
  ...
  useEffect(() => {
    if (data?.job) {
      ...
      setSelectedSkills(j.skillsRequired);
      setLocation(j.jobLocation || "");
      setQuestions(j.questions || []);
      setDegree(j.education || "");
      setDegreeLevel(j.degreeLevel || "any");
      setSalary(j.monthlySalary || { min: "", max: "" });
      setExperience(j.experience || { min: "", max: "" });
    }
  }, [data]);
  ...
  if (!(role === "employer" || role === "recruiter")) {
    return <AccessDenied title1="employer" title2="recruiter" />;
  }
  ...
  const initialValues = {
    ...
    monthlySalary: job.monthlySalary || { min: "", max: "" },
    experience: job.experience || { min: "", max: "" },
    degreeLevel: job.degreeLevel || "any",
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      const response = await editJob(payload).unwrap();
      ...
      toast.error(err?.data?.message || "Error updating job.");
    }
  };

  return (
    <div className="w-full py-6 px-4 md:px-16 lg:px-28">
      <h1 className="text-2xl font-semibold mb-6">Edit Job Details</h1>

      <SEOModal slug="edit-job" />
          ...
          <Form className="space-y-6">
            ...
            <div>
              ...
              <JobTitleSearchBar
                ...
                onJobTitleSelect={(selected) => {
                  setJobTitle(selected.label);
                  setFieldValue("jobTitle", selected.label);
                }}
              />
              <ErrorMessage
                name="jobTitle"
            ...
            <div>
              ...
              <div className="flex rounded-md shadow-sm">
                <Field
                  id="url"
                  name="url"
                  placeholder="url-of-this-job"
                  className={inputClass}
                  pattern="^[a-z0-9]+(?:-[a-z0-9]+)*$"
                  title="Only lowercase letters, numbers, and hyphens allowed (no spaces)"
                />
              </div>
              <ErrorMessage
                name="url"
            ...
            <div className="text-sm font-semibold">
              <h1>
                Salary <span className="text-xs font-thin">(Per Month)</span>
              </h1>
                  ...
                  questions.map((q, index) => (
                    ...
                    >
                      <div className="space-y-2">
                        ...
                        {q.type === "text" && (
                          <p className="italic text-gray-600">Text answer</p>
                        )}

                        {q.type === "single-choice" && (
                          <div>
                            {q.options.map((option, i) => (
                              <label key={i} className="block cursor-default">
                                <input
                                  type="radio"
                                  name={`question${index}`}
                                  disabled
