...
const validationSchema = Yup.object({
  name: Yup.string()
    ...
    .matches(
      /^[a-zA-Z0-9 ]+$/,
      "Skill name should only contain letters, numbers, and spaces"
    ),
});
...
const Industry = () => {
  ...
  const [bulkUploadIndustries, { error: bulkUploadIndustriesError }] =
    useBulkUploadIndustriesMutation();
  const [addIndustry, { error: addIndustryError }] = useAddIndustryMutation();
  const [updateIndustry, { error: updateIndustryError }] =
    useUpdateIndustryMutation();
  const [deleteIndustry, { error: deleteIndustryError }] =
    useDeleteIndustryMutation();
  ...
  const handleAddOrUpdateOrDeleteIndustry = async (values, actionType) => {
    try {
      let response;
      if (actionType === "edit" && editingIndustry) {
        response = await updateIndustry({
          ...editingIndustry,
          name: values.name,
        });
      } else if (actionType === "add") {
        response = await addIndustry({ name: values.name });
      } else if (actionType === "delete") {
        response = await deleteIndustry(editingIndustry._id);
      } else {
        ("");
      }
  ...
  const handleBulkUpload = async (e) => {
    ...
    try {
      ...
      const response = await bulkUploadIndustries(formData).unwrap();
  ...
  const initialValues = { name: editingIndustry ? editingIndustry.name : "" };
    ...
    <div className="flex flex-col w-full gap-8 p-4">
          ...
          industries.map((industry) => (
            ...
            >
              <div className="flex flex-col items-center">
                ...
                <div className="text-sm text-gray-500">{industry.level}</div>
              </div>
      ...
      {isModalOpen && (
        <GroupedFilterModal
          ...
          data={editingIndustry}
          onAddOrUpdateOrDelete={handleAddOrUpdateOrDeleteIndustry}
          error={addIndustryError || updateIndustryError || deleteIndustryError}
